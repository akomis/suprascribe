...
import { getSeoPagesForPost } from '@/lib/config/linkGraph'
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildMetadata({
    title: post.title,
    // `| Suprascribe` rather than `| Suprascribe Blog`: the five extra characters came out of
    // the title's own budget in the SERP, and the article's own words earn their place first.
    absoluteTitle: `${post.metaTitle ?? post.title} | Suprascribe`,
...
export default async function BlogArticlePage({ params }: Props) {
  ...
  // `relatedPageLinks` pins the pages this post must link to; the link graph tops the row up with
  // the marketing pages that share a topic and have the fewest inbound links, so no page in
  // app/(seo) is left reachable only from its siblings.
  const relatedResources = [
    { href: '/login?tab=signup', label: 'Start for Free' },
    { href: '/demo', label: 'See the Demo' },
    ...getSeoPagesForPost(post),
  ]
