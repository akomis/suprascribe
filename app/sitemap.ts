...
export default function sitemap(): MetadataRoute.Sitemap {
  return [
    ...
    ...BLOG_CATEGORIES.map((category) => ({
      url: `https://www.suprascribe.com/blog/category/${category.id}`,
      lastModified: new Date(
        blogPosts
          .filter((post) => getPostCategories(post.topics).includes(category.id))
          .reduce((latest, post) => {
            const d = post.updatedAt ?? post.publishedAt
            return d > latest ? d : latest
          }, '2020-01-01'),
    ...
    ...seoPages.map((page) => ({
      url: `https://www.suprascribe.com${page.path}`,
