...
export function DemoSubscriptionDetailsDialog({
  ...
  const deleteSubscriptionMutation = useDemoDeleteSubscription()
  const updateSubscriptionMutation = useDemoUpdateSubscription()
  ...
  const handleMarkAsCancelled = async () => {
    ...
    try {
      await updateSubscriptionMutation.mutateAsync({
        ...
        data: {
          serviceName: mostRecentSubscription.subscription_service?.name || '',
          serviceUrl: mostRecentSubscription.subscription_service?.url || undefined,
          startDate: mostRecentSubscription.start_date || '',
          endDate: today,
          autoRenew: false,
          price: mostRecentSubscription.price || 0,
          currency: mostRecentSubscription.currency as CurrencyCode,
          paymentMethod: mostRecentSubscription.payment_method || undefined,
        },
      })
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader>
          <div className="flex flex-row items-start justify-between gap-4 pt-4">
            ...
            {!isPast && subscription.subscription_service?.unsubscribe_url && (
      ...
      <Dialog open={showUnsubscribeConfirm} onOpenChange={setShowUnsubscribeConfirm}>
        <DialogContent className="max-w-xl sm:min-w-sm">
          <DialogHeader>
