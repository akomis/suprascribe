...
// CustomDot component defined outside of SubscriptionHistory to avoid recreating on each render
type CustomDotProps = {
  cx?: number
  cy?: number
  payload?: {
...
function CustomDot({ cx, cy, payload, onEdit }: CustomDotProps) {
  ...
  if (!shouldShow || cx === undefined || cy === undefined) {
    return null
  }
    ...
    <g className="fade-on-mount">
      {paymentMethod && !isEndDate ? (
        ...
        <circle
          ...
          onMouseLeave={() => setIsHovered(false)}
          className={
            isEndDate ? 'pointer-events-auto cursor-default' : 'pointer-events-auto cursor-pointer'
          }
        />
      )}
      <circle
        ...
        className={
          isEndDate
            ? 'fill-neutral-300 stroke-neutral-200 dark:fill-gray-700 dark:stroke-gray-500 pointer-events-none transition-none'
            : 'fill-gray-700 stroke-gray-700 dark:fill-white dark:stroke-white pointer-events-none transition-all duration-200 ease-in-out'
        }
        strokeWidth={2}
      />
    </g>
...
function SubscriptionHistory({ subscriptions, onEdit }: SubscriptionHistoryProps) {
  ...
  // Memoize the dot renderer to prevent unnecessary re-renders
  const renderDot = React.useCallback(
    (props: unknown) => {
      const { key, ...restProps } = props as CustomDotProps & { key?: string | number }
      return <CustomDot key={key} {...restProps} onEdit={onEdit} />
    },
    [onEdit],
  )
      ...
      >
        <ResponsiveContainer width="99%" height="100%" debounce={50}>
          <LineChart data={chartData} margin={{ left: 5, right: 5, top: 5, bottom: 0 }}>
            ...
            />
            {currentMonthPeriod && chartData.some((d) => d.period === currentMonthPeriod) && (
              <ReferenceLine
