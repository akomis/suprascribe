...
function SubscriptionControls({
  ...
  // Gate on the injected flag, not ProGate's own tier read: the demo grants search/sort
  // without a tier, so an unconditional ProGate would lock it for every demo visitor.
  const gate = (control: React.ReactNode) =>
    hasSearchSort ? control : <ProGate feature="search_sort_group">{control}</ProGate>
