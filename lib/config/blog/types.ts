...
export interface BlogPost {
  ...
  description: string
  /**
   * Short title for the `<title>` tag, when `title` is too long to survive a search result.
   * Google truncates at roughly 60 characters including the "| Suprascribe" suffix, so this
   * should stay at or under 44. Falls back to `title`, which stays long for the page's own
   * `<h1>` - the heading has room that the SERP does not.
   */
  metaTitle?: string
  /**
   * Short description for the meta description tag, when `description` runs past the ~155
   * characters a snippet shows. Falls back to `description`, which stays as the deck rendered
   * under the heading.
   */
