...
interface UseMicrosoftDiscoveryReturn {
...
export function useMicrosoftDiscovery(): UseMicrosoftDiscoveryReturn {
  ...
  useEffect(() => {
    ...
    if (shouldDiscover && !hasCalledWebhook.current) {
      fetch('/api/discovery/token?provider=microsoft')
  ...
  const callDiscoveryApi = async (token: string) => {
    ...
    try {
      const response = await fetch('/api/discovery/discover/microsoft', {
        method: 'POST',
