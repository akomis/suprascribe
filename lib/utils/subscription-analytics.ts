...
function sumMonthly(subs: Subscription[]): number {
  return subs.reduce((sum, s) => sum + s.monthlyCost, 0)
}

function nextExpiring(subs: Subscription[], today = new Date()): Subscription[] {
...
function getActiveSubscriptionsForMonth(subs: Subscription[], today = new Date()): Subscription[] {
  const day = new Date(today)
  day.setHours(0, 0, 0, 0)
  return subs.filter((s) => isActiveOnDate(s, day))
}

function getPastSubscriptionsForYear(
  subs: Subscription[],
  year: number,
  today = new Date(),
): Subscription[] {
...
function getActiveMonthsInYear(sub: Subscription, year: number, today = new Date()): number {
  const effectiveStart = new Date(
    Math.max(new Date(sub.startDate).getTime(), new Date(year, 0, 1).getTime()),
  )
  const effectiveEnd = new Date(
...
function _calculateSpentThisYear(sub: Subscription, today = new Date()): number {
  return sub.monthlyCost * getActiveMonthsInYear(sub, today.getFullYear(), today)
}

function _calculateForecastThisYear(sub: Subscription, today = new Date()): number {
