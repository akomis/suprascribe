...
async function upsertService(
...
): Promise<{ serviceId: number } | { error: string; status: number }> {
  ...
  if (existing) {
    ...
    if (Object.keys(updateData).length > 0) {
      ...
      if (updateError) {
        return { error: `Error updating service: ${updateError.message}`, status: 500 }
      }
    }
  ...
  if (createError) {
    ...
    if (retryError || !retried) {
      return {
        error: `Error resolving service after conflict: ${retryError?.message}`,
        status: 500,
      }
    }
